...
class SVMServicer(SVM_pb2_grpc.SVMServicer):
    ...
    def __init__(self, config_file='config.json', is_worker=True):
        ...
        if is_worker:
            ...
            self.data, self.target, self.du = load_data(
                config.test_data_path, nb_sample=config.val_data_size, proba_sample=config.proba_sample)
    ...
    def SendNodeInfo(self, request, context):
        ...
        self.data = self.data[int(data_len*self.worker_nb):int(data_len*self.worker_nb+data_len)]
